...
class String {
		 ...
		 */
//		char* data = new char[1] {0};new int[10], std::default_delete<int[]>()
//		char* da_ = new char[1] {0};
		shared_ptr<char[]> data =
			shared_ptr<char[]>(new char[1] {0} , [](char* p) {delete p; }, std::allocator<char*>());
		/**
		 * 每个String实例都可以有唯一一个对应的builder,辅助拼接
		 * 要使用指针类型, 因为不是每一个String都会使用一个StringBuilder辅助
		 * 使用单例模式,每个String只有唯一一个对应的builder, String释放时,其也被释放
		 */
		StringBuilder* _builder = nullptr;
	
		/**
		 * 标识这个String是否是临时变量
		 * 若是, 则说明这个对象仅仅只是过渡使用, char* 指针不能先被释放掉
		 * 因为可能还要用, 详情看析构
		 */
		bool temp = false;
	...
	public:
		...
		inline bool contain(const String& sub);
		...
		inline String replaceWith(const String& search, const String& with = "", int direction = LEFT);
		inline String replaceFirstWith(const String& search, const String& with = "");
		inline String replaceLastWith(const String& search, const String& with = "");
		inline String replaceAllWith(const String& search, const String& with = "");
...
shared_ptr<char*> String::copyAndReplace(const char* source, int length) {
	if (length < 1) {
		return data;
	}
...
String::String(const String& source) {
	cout<<"copy from "<<&source<<" "<<this<<endl;
	// 这个是个拷贝构造,用到该构造, 很有可能就是在传参以及返回时, 所以该
	// 串应该只是一个临时变量,其中的char*指针只做传递之用
	temp = true;
...
String String::replace(const String& search, const String& with, bool merge, int direction) {
	...
	return String(resultChar, resultLength);
}
