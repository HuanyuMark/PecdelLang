...
class StringBuilder {
	...
	public:
		...
		StringBuilder& reserve(ull capacity) {
			menbers.reserve(capacity);
			return *this;
		}

		inline StringBuilder& operator+(const String& target);
		inline StringBuilder& operator+(const Object& target);
...
//		shared_ptr<char[]> data = getIinitialzedSharedPointer(/*lastAddress*/); //make_shared<char[]>(nullptr); //make_shared<char*>(new char[1] {0});
		 ...
		 */
		static Array<int> const getPrefixTable(const String& search);
		static Array<int> const getSuffixTable(const String& search);
...
//			cout << "target: " << p << "count: " << ++releaseCount << "address update? " << (p == lastAddress) << endl;
		 ...
		 */
		inline const SharedPtr<char*> getIinitialzedSharedPointer(char* pointer = new char[1] {0})  {
//			lastAddress = new char[1] {0};
	...
	public:
		...
		String(const StringBuilder& builder); // 会影响到builder里的vector
		String(Object& obj);
...
//		String(const O& obj);
		...
		// 返回所有相同子串的索引
		Array<int> allIndexOf(const String& search);
...
// 拷贝构造, 和默认的拷贝构造相同
String::String(const String& source): Object(source), _length(source._length), data(source.data) {
...
Array<int> String::allIndexOf(const String& search) {
//	vector<int>* result = new vector<int>();
	...
	Array<int> prefixTable = getPrefixTable(search);
...
int String::lastIndexOf(const String& search) {
	...
	Array<int>  suffixTable = getSuffixTable(search);
...
Array<int> const String::getPrefixTable(const String& search) {
	/**
...
//	int* table = new int[tableLength];
	Array<int> table = Array<int>(tableLength);
...
Array<int> const String::getSuffixTable(const String& search) {
	const char* compChar = search.toChars();
...
String String::replace(const String& search, const String& with, bool merge, int direction, int replaceIndex) {
	...
	// 根据方向来选择使用哪种匹配方法
	if (direction == LEFT) {
		checkReplaceIndex(replaceIndex, [&]()-> bool {return (replaceIndex < 0) || replaceIndex == INT_MAX;}, "'String::LEFT'");
		startIndex = indexOfthe(search, replaceIndex);
	} else if (direction == RIGHT) {
		checkReplaceIndex(
		    replaceIndex,
		    [&]()-> bool {return (replaceIndex > (INT_MAX - 1000));},
		    "'String::RIGHT'");
		startIndex = lastIndexOf(search);
	} else {
		// checkReplaceIndex(replaceIndex, [&]()-> bool {return (replaceIndex > 0);}, "'String::BOTH'");
...
String String::replaceAll(const String& search, const String& with, bool merge) {
	Array<int> allStartIndexes = allIndexOf(search);
	int length = allStartIndexes.length();
	int resultLength = _length - (search._length * length) + (with._length * length);
...
String::String(const StringBuilder& builder) {
	String result = ((StringBuilder&)builder).build();
	data = result.data;
	_length = result._length;
}

//template<typename O>
//String::String(const O& obj){
////	type_info info = typeid(obj);
////	typeid(obj).;
////	cout<<"typeid(obj): "<< <<endl;
////	String result = ((O&)obj).toString();
//	String result = "TTTTT";
//	data  = result.data;
//	_length = result._length;
//}
...
String::String(Object& obj) {
	cout << "String(Object& obj)" << (void*)&obj << endl;
//	Object* objPointer = &((Object&) obj);
//	cout<<"objPointer: "<<objPointer<<endl;
//	cout<<"objPointer.hashcode(): _"<<objPointer->hashCode()<<endl;
//	cout<<"typeid(obj).name(): "<<typeid(obj).name()<<endl;
//	Object& temp = (Object&)obj;
//	Object temp = obj;
//	Object* pointer = &obj;
//	void* tp = obj;
//	cout<<"ready to copy obj"<<endl;
	Object* temp = &obj;
//	cout<<"copyed: "<<typeid(temp).name()<<endl;
	String result = temp->toString();//temp.toString();
	data = result.data;
	_length = result._length;
}
...
StringBuilder& StringBuilder::operator+(const Object& target) {
	cout << "operator+(const Object& target)" << endl;
	String result = ((Object&)target).toString();
	menbers.push_back(result);
	return *this;
}
...
String StringReplacer::replace() {
	if (_all) {
		return target->replaceAll(_search, _with, _merge);
	}
...
String Array<T>::toString() {
	StringBuilder builder;
	builder.reserve((_length << 1) - 1 + 4);
	builder + "[ ";

	T* arr = data.get();

	if (_length > 0 ) {
//		cout<<"before push: _"<<(void*)&arr[0]<<((Object&)arr[0]).toString().toChars()<<endl;
//		cout<<"string: "<<((String&)arr[0]).toString().toChars()<<endl;
		builder + arr[0];
	}
